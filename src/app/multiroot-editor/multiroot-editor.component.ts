...
import { ModalListOfSpeakersComponent, LspDataToEditor } from './modal-list-of-speakers/modal-list-of-speakers.component';
...
export class MultirootEditorComponent implements AfterViewInit {
  ...
  onContextMenuResolve($event) {
    ...
    console.log('#### onContextMenuResolve this.Editor.model.document.selection:', this.Editor.model.document.selection);
    ...
    console.log('#### newEditor.model.document.selection.getSelectedElement():', this.Editor.model.document.selection.getSelectedElement());
    console.log('#### newEditor.view.documentselection.getSelectedElement():', this.Editor.model.document.selection.getSelectedElement());
    console.log('#### newEditor.model.document.selection.getFirstPosition():', this.Editor.model.document.selection.getFirstPosition());
    ...
    if (viewElement.name === 'span' && viewElement.getAttribute('data-type') === 'var_date') {
      ...
      modalRef.result.then((result) => {
        if (result) {
          ...
          console.log('#### EDITOR MODAL result string:', result.year + '/' + result.month + '/' + result.day);
          this.Editor.model.change(writer => {
            console.log('#### EDITOR MODAL result string:', result.year + '/' + result.month + '/' + result.day);
            writer.setAttribute('data-content', result.year + '/' + result.month + '/' + result.day, modelElement);
          });
        }
      });
    } else if (viewElement.name === 'span' && viewElement.getAttribute('data-type') === 'var_time') {
      ...
      modalRef.result.then((result) => {
        if (result) {
          console.log('#### EDITOR MODAL result:', result);
          console.log('#### EDITOR MODAL result string:', result.hour + ':' + result.minute);
          this.Editor.model.change(writer => {
            console.log('#### EDITOR MODAL result string:', '"' + result.hour + ':' + result.minute + '"' );
            writer.setAttribute('data-content', result.hour + ':' + result.minute, modelElement);
          });
        }
      });
    } else if (viewElement.name === 'span' && viewElement.getAttribute('data-type') === 'var_str') {
    ...
    } else if (viewElement.name === 'span' && viewElement.getAttribute('data-type') === 'var_sp') {
      const modalRef = this.modalService.open(ModalListOfSpeakersComponent, { size: 'xl' });
